...
echo "<table id='featuresetTable' class='display' cellspacing='0' width='100%'>";
echo "<thead><tr>";
// Print featureset header
$index = 1;
foreach ($view_featuresetHeader as $column) {
    echo "<th>$column</th>";
    $index++;
}

echo "<tbody>";
foreach ($view_featureset as $recordNumber => $row) {
    $index = 1;
    // Print featureset values
    $ds = "<tr>";
